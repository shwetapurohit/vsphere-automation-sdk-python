...
class CreateProtectionGroup(object):
    ...
    def run(self):
        ...
        task = self.ssClient.clusters.ProtectionGroups.create_task(clusterId, spec=spec)

        while True:
            task_info = self.ssClient.Tasks.get(task.get_task_id())
    ...
    def cleanup(self):
        pgs_info = self.ssClient.clusters.ProtectionGroups.list(self.clusterId)
        ...
        for pg_info in pgs_info.items:
            if pg_info.info.name == testbed.config["PG_NAME"] and not testbed.config["LOCK"]:
                ...
                self.ssClient.clusters.ProtectionGroups.delete_task(
